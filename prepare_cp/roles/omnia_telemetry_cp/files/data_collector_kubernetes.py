...
def get_kubectl_get_pods():
    ...
    output=invoke_commands.call_command('sudo /usr/local/bin/kubectl get pods -A -o json')
...
def get_kubectl_get_nodes():
    ...
    index_status=4
    output=invoke_commands.call_command('sudo /usr/local/bin/kubectl get nodes -o json')
    if output is not None:
...
def get_kubectl_get_cs():
    ...
    output=invoke_commands.call_command('sudo /usr/local/bin/kubectl get --raw=/healthz?verbose')
    if output is not None:
        ...
        common_logging.log_error("data_collector_kubernetes:get_kubectl_get_cs","kubectl get --raw=/healthz?verbose command output is None")
